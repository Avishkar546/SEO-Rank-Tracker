...
export default function RankTracker() {
  ...
  const fetchKeywords = async () => {
    try {
      const res = await api.get("/api/v1/rankkeyword/list");
      console.log("Fetched keywords:", res);
      if (res.data.success) {
        setKeywords(res.data.data);
      } else {
        console.error("Failed to fetch keywords:", res.data.message);
      }
    } catch (error) {
  ...
  const handleAdd = async (e: React.SubmitEvent) => {
    ...
    try {
      const res = await api.post("/api/v1/rankkeyword/add", {
        keyword: newKeyword,
        url: newUrl,
      });
      if (res.data.success) {
        ...
        const pollInterval = setInterval(async () => {
          try {
            const check = await api.get(`/api/v1/rankkeyword/${id}`);
  ...
  const handleRefresh = async (id: string) => {
    setRefreshing(id);
    try {
      const res = await api.post(`/api/v1/rankkeyword/${id}/refresh`);
      setKeywords((prev) =>
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    try {
      const res = await api.delete(`/api/v1/rankkeyword/${id}`);
      if (res.data.success) {
  ...
  const handleToggle = async (id: string) => {
    ...
    try {
      ...
      if (!kw) return;
      const res = await api.put(`/api/v1/rankkeyword/${id}/toggle`);
      console.log("Toggle response:", res);
      if (res.data.success) {
        setKeywords((prev) =>
          prev.map((k) =>
            k._id === id ? { ...k, active: res.data.active } : k,
          ),
        );
      }
    } catch (error) {
