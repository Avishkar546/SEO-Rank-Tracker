...
export default function History() {
    ...
    const fetchAnalyses = async () => {
        ...
        try {
            const { data } = await api.get(
                `/api/v1/analysis/list?page=${page}&limit=12`,
            );
            if (data.success) {
            ...
            const errorMessage = error.response?.data?.message || error.message || "An unexpected error occurred while fetching analysis history";
    ...
    const handleDelete = async (id: string) => {
        ...
        setDeleting(id);
        try {
            const res = await api.delete(`/api/v1/analysis/${id}`);
            console.log("delete response: ", res);
            if (res.status === 204 || res.status === 200) {
                setAnalyses((prev) => prev.filter((a) => a._id !== id));
                toast.success("Analysis deleted successfully");
            } else {
                toast.error(res.data?.message || "Failed to delete analysis");
            }
        } catch (error: any) {
            console.error("Error deleting analysis:", error);
            const errorMessage = error.response?.data?.message || error.message || "An unexpected error occurred while deleting the analysis";
            toast.error(errorMessage);
        } finally {
            setDeleting(null);
        }
