...
export default function Analyze() {
  ...
  const handleAnalyze = async (submitUrl?: string) => {
    ...
    try {
      // step 0: Connection to browser
      setCurrentStep(0);
      const res = await api.post("/api/v1/analysis", { url: targetUrl.trim() });
      console.log("Analysis start response:", res);
      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          const statusRes = await api.get(`/api/v1/analysis/${id}`);
          const analysis = statusRes.data.analysis;
