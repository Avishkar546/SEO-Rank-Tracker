...
export default function Report() {
    ...
    const fetchAnalysis = async () => {
        try {
            const res = await api.get(`/api/v1/analysis/${id}`);
            if (!res || !res.data.success) {
                setError("Analysis not found. Please check the URL or try again.");
                throw new Error(res.data.message || "Failed to fetch analysis");
            }
            if(res.data.analysis.status === "pending") {
